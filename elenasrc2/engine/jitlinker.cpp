...
ref_t JITLinker :: parseMessage(ident_t reference)
{
   ...
   if (index != -1) {
      ...
      // resolve signature
      signatureId = (ref_t)(size_t)_loader->resolveReference(signature, 0);
   }
   else {
      // if it is a predefined verb
      if (reference[0] == '#') {
      ...
      else signatureId = (ref_t)(size_t)_loader->resolveReference(reference, 0);
   }
