...
public:
   ...
   // reads a native machine word (pointer-width); used only by in-memory
   // structures that are never persisted, e.g. the compiler syntax tree
   size_t getSize()
   {
      size_t value = 0;
      read((void*)&value, sizeof(size_t));
...
public:
   ...
   // writes a native machine word (pointer-width); in-memory structures only
   void writeSize(size_t value)
   {
      write(&value, sizeof(size_t));
   }
...
public:
   ...
   // inserts a native machine word (pointer-width); in-memory structures only
   void insertSize(size_t position, size_t value)
   {
      _memory->insert(position, (char*)&value, sizeof(size_t));
