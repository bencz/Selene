...
public:
   ...
   public:
      ...
      void operator = (LexicalType type)
      {
         ...
         size_t prevPos = tree->_reader.Position();

         tree->_reader.seek(position - 2 * sizeof(size_t));
         *(size_t*)(tree->_reader.Address()) = (size_t)type;
      ...
      void setArgument(ref_t argument)
      {
         ...
         tree->_reader.seek(position - sizeof(size_t));
         *(size_t*)(tree->_reader.Address()) = argument;
      ...
      void insertNode(LexicalType type, ref_t argument = 0)
      {
         tree->insertNode(position, type, argument);
      }

      void appendNode(LexicalType type, ref_t argument = 0)
      {
         Node lastNode = lastChild();

         tree->insertNode(lastNode.position + 2 * sizeof(size_t), type, argument);
      }
...
public:
   ...
   Node insertNode(size_t position, LexicalType type, ref_t argument);
