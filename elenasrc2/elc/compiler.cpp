      ...
      if (size > 0) {
         ...
         switch (flags & elDebugMask)
         {
            case elDebugDWORD:
               ...
               scope.writer->appendNode(lxTerminal, (ref_t)terminal.value);
            ...
            default:
               ...
               scope.writer->appendNode(lxTerminal, (ref_t)terminal.value);
               scope.writer->appendNode(lxLevel, variable.param);
               scope.writer->appendNode(lxClassName, (ref_t)scope.moduleScope->module->resolveReference(classReference));
               scope.writer->closeNode();
               break;
         }
      }
...
void Compiler :: compileDynamicDefaultConstructor(MethodScope& scope, ClassScope& classClassScope)
{
   ...
   if (test(classScope->info.header.flags, elStructureRole)) {
      ...
      switch((int)classScope->info.size) {
         case -1:
