...
int ParserTable :: defineSymbol(const TCHAR* terminal)
{
   return _symbols.get(terminal);
}

bool ParserTable :: registerSymbol(int symbol, const TCHAR* value)
{
...
// method returns false if the syntax ambigous
bool ParserTable :: generate()
{
   ...
   for (SyntaxHash::Iterator rule = _syntax.start() ; !rule.Eof() ; nextKey(rule)) {
      ...
      if (*rule == nsEps) {
         ...
         if (!test(*rule, mskTerminal)) {
		   ...
		   else if (!copySubSet(_syntax, _table, rule.key(), tableKey(l_symbol, *rule)))
            return false;
      }
   }
   return true;
...
{
  // save terminals
   _symbols.write(writer);	// !! better to save only terminal symbols
