...
int x86LabelHelper :: fixShortLabel(size_t labelPos)
{
   ...
   else *(char*)(&(*code->getSection())[labelPos]) = offset;
...
int x86LabelHelper :: fixNearLabel(size_t labelPos)
{
   ...
   (*code->getSection())[labelPos] = offset;
...
void x86LabelHelper :: fixLabel(const int label)
{
   ...
   while (!it.Eof() && it.key() == label) {
      ...
      // get jump byte
      int opcode = (*code->getSection())[labelPos - 1];
...
void x86LabelHelper :: fixJumps(int position, int size)
{
   ...
   while (!it.Eof()) {
      ...
      // in case of back jump
         ...
         if (abs(offset) < 0x80) {
            *(char*)(&(*code->getSection())[labelPos]) = offset;
         }
         else {
            // if we are unlucky we must change jump from short to near
            ...
            else (*code->getSection())[labelPos] = offset;
         }
         (*it).offset = offset;
      }
...
{
   int opcode = (*code->getSection())[position - 1];
   ...
   if ((unsigned char)opcode == 0xEB) {
      ...
      *(unsigned char*)(&(*code->getSection())[position - 1]) = 0xE9;
      (*code->getSection())[position] = offset;

      shiftLabels(position, 1, 3);
      fixJumps(position + 2, 3);
   }
   else {
      *(char*)(&(*code->getSection())[position - 1]) = 0x0F;
      *(char*)(&(*code->getSection())[position]) = (char)opcode + 0x10; // to change from short to near
      code->insertDWord(position + 1, offset);

      shiftLabels(position, 2, 4);
