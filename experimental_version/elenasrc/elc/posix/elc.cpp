...
namespace
{
   ...
   // Foreign function declarations travel as MODULE METADATA: the compiler
   // writes each `#define[external, ...] name = pkg'func.` as a native data
   // section under the same $package name the call sites map, holding
   // "sym=...;args=...;ret=...". Collected in a PRE-PASS over the whole
   // closure, because a call site may sit in a module that translates before
   // the module declaring it.  docs/plan/18-ffi-design.md
   bool collectForeignDeclarations(LLVMGenerator& generator, const char* path)
   {
      LocalString<0x200> modulePath;
      modulePath.copy(path);
      ...
      FileReader file(modulePath, feRaw);
      if (!file.isOpened())
         return true;                              // reported by the translate pass
      ...
      for (ref_t r = 1 ; r < 0x1000 ; r++) {
         ...
         std::string plain = narrowName(refName);
         if (plain.compare(0, 9, "$package'") != 0)
            continue;

         Section* meta = module.mapSection(r | mskNativeDataRef, true);
         if (!meta || meta->Length() == 0)
            continue;

         std::string spec((const char*)meta->getArray(), meta->Length());

         std::string sym, argTypes, retType = "void";
         size_t at = 0;
         while (at <= spec.size()) {
            ...
            std::string field = spec.substr(at, end - at);
            ...
            if (eq != std::string::npos) {
               ...
               if (k == "sym")       sym = v;
               else if (k == "args") argTypes = v;
               else if (k == "ret")  retType = v;
               // "lib" is informational under static linking; it starts
               // mattering with dlopen-style loading (plan 18 section 11.3).
            }
            at = end + 1;
         }

         const char* error = NULL;
         if (!generator.addForeignFunction(plain.c_str(), sym.c_str(),
                                           argTypes.c_str(), retType.c_str(),
                                           &error))
         {
            printf("ffi invalido em '%s': %s\n", path, error ? error : "?");
            return false;
         }

         printf("ffi: %s -> %s(%s)%s%s\n", plain.c_str(), sym.c_str(),
                argTypes.c_str(), retType == "void" ? "" : " ret ",
                retType == "void" ? "" : retType.c_str());
      }
      return true;
   }
   ...
   bool linkProject(_ELC_::Project& project)
   {
      ...
      for (std::set<std::string>::iterator m = modules.begin() ;
           m != modules.end() ; ++m)
      {
         if (!collectForeignDeclarations(generator, m->c_str()))
            return false;
      }
...
int main(int argc, char* argv[])
{
   ...
   if (argc >= 3 && compstr(argv[1], "--llvm-translate")) {
      ...
      for (size_t pathAt = 0 ; pathAt < modulePaths.size() ; pathAt++) {
         if (!collectForeignDeclarations(generator, modulePaths[pathAt])) {
            delete[] args; return -1;
         }
      }
