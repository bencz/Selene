...
void Compiler :: compileExternalDeclaration(DNode hints, TerminalInfo reference,
                                            ModuleScope& scope)
{
   // #define[external, lib:c, sym:write, args:i32'str'usize, ret:isize] ...
   //
   // A foreign function declaration, in the language, riding the hint
   // grammar that already exists. It compiles to MODULE METADATA -- a native
   // data section under the same $package name the call sites use -- and the
   // link step turns every call into a typed, marshalled call to the C
   // symbol. The apostrophes of the args reference stand in for commas,
   // which the hint grammar reserves.  docs/plan/18-ffi-design.md
   String spec(0x100);

   while (hints == nsHint) {
      const TCHAR* name = hints.Terminal();

      if (!compstr(name, HINT_EXTERNAL)) {
         DNode valueNode = hints.firstChild();
         if (valueNode != nsHintValue) {
            scope.raiseWarning(wrnUnknownHint, hints.Terminal());
            hints = hints.nextNode();
            continue;
         }
         const TCHAR* value = valueNode.Terminal();

         if (compstr(name, HINT_FFI_SYM)) {
            spec.append(_T("sym="));
            spec.append(value);
            spec.append(_T(";"));
         }
         else if (compstr(name, HINT_FFI_ARGS)) {
            spec.append(_T("args="));
            // i32'str'usize -> i32,str,usize
            for (size_t i = 0 ; value[i] ; i++)
               spec.append(value[i] == '\'' ? (TCHAR)',' : value[i]);
            spec.append(_T(";"));
         }
         else if (compstr(name, HINT_FFI_RET)) {
            spec.append(_T("ret="));
            spec.append(value);
            spec.append(_T(";"));
         }
         else if (compstr(name, HINT_FFI_LIB)) {
            spec.append(_T("lib="));
            spec.append(value);
            spec.append(_T(";"));
         }
   ...
   // The metadata lands under the SAME reference the call sites map --
   // $package'<name> -- in a native data section, canonical bytes.
   ref_t ref = scope.module->mapReference(
      ReferenceName(PACKAGE_MODULE, reference.value));

   SectionWriter writer(scope.module->mapSection(ref | mskNativeDataRef, false));
   for (size_t i = 0 ; i < getlength((const TCHAR*)spec) ; i++)
      writer.writeByte((unsigned char)((const TCHAR*)spec)[i]);
}
   ...
   while (member != nsNone) {
      ...
      switch (member) {
         case nsShortcut:
         {
            ...
            // a foreign function declaration: metadata, not a forward
            else if (hints == nsHint && compstr(hints.Terminal(), HINT_EXTERNAL)) {
               compileExternalDeclaration(hints, reference, scope);
            }
