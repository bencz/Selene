...
public:
   ...
   // Declares a foreign function ([ffi] in the configuration chain): calls
   // to the native `name` compile into a typed call to the C symbol instead
   // of the two-slot native convention. Types, comma-separated:
   //   i8 i16 i32 i64 u8 u16 u32 u64   unboxed from the object payload
   //   isize usize                     i32 payload widened to the target word
   //   f64                             the payload double
   //   ptr                             the object pointer (= its payload)
   //   str                             UTF-8 bytes past the literal's length
   // Return: void, or any of the above (phase 1 discards it; failures carry
   // it once fail: takes an argument). See docs/plan/18-ffi-design.md.
   bool addForeignFunction(const char* name, const char* symbol,
                           const char* argTypes, const char* returnType,
