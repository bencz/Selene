...
namespace
{
   ...
   struct Impl
   {
      ...
      // --- foreign functions ([ffi] in the configuration chain) ---
      //
      // A declared native compiles into a typed call to the C symbol; the
      // marshalling wrapper is emitted on first use under the native's own
      // selene.native name, so nothing else changes -- the C stub simply
      // stops existing.  docs/plan/18-ffi-design.md
      enum FFIType
      {
         ffiVoid, ffiI8, ffiI16, ffiI32, ffiI64,
         ffiU8, ffiU16, ffiU32, ffiU64,
         ffiISize, ffiUSize, ffiF64, ffiPtr, ffiStr
      };
      ...
      std::map<std::string, FFIDecl> foreign;   // keyed by full native name

      static bool parseFFIType(const std::string& text, FFIType& out)
      {
         if (text == "void")  { out = ffiVoid;  return true; }
         if (text == "i8")    { out = ffiI8;    return true; }
         if (text == "i16")   { out = ffiI16;   return true; }
         if (text == "i32")   { out = ffiI32;   return true; }
         if (text == "i64")   { out = ffiI64;   return true; }
         if (text == "u8")    { out = ffiU8;    return true; }
         if (text == "u16")   { out = ffiU16;   return true; }
         if (text == "u32")   { out = ffiU32;   return true; }
         if (text == "u64")   { out = ffiU64;   return true; }
         if (text == "isize") { out = ffiISize; return true; }
         if (text == "usize") { out = ffiUSize; return true; }
         if (text == "f64")   { out = ffiF64;   return true; }
         if (text == "ptr")   { out = ffiPtr;   return true; }
         if (text == "str")   { out = ffiStr;   return true; }
         return false;
      }
...
bool LLVMGenerator :: addForeignFunction(const char* name, const char* symbol,
                                         const char* argTypes,
                                         ...
                                         const char** errorMessage)
{
   ...
   if (decl.symbol.empty()) {
      impl->lastError = std::string("ffi '") + name + "': no symbol";
      if (errorMessage) *errorMessage = impl->lastError.c_str();
      return false;
   }

   std::string types = argTypes ? argTypes : "";
   size_t at = 0;
   while (at < types.size()) {
      size_t comma = types.find(',', at);
      if (comma == std::string::npos)
         comma = types.size();

      std::string one = types.substr(at, comma - at);
      if (!one.empty()) {
         Impl::FFIType kind;
         if (!Impl::parseFFIType(one, kind) || kind == Impl::ffiVoid) {
            impl->lastError = std::string("ffi '") + name
                            + "': unknown argument type '" + one + "'";
            if (errorMessage) *errorMessage = impl->lastError.c_str();
            return false;
         }
         decl.args.push_back(kind);
      }
      at = comma + 1;
   }

   std::string ret = returnType ? returnType : "void";
   if (!Impl::parseFFIType(ret, decl.ret)) {
      impl->lastError = std::string("ffi '") + name
                      + "': unknown return type '" + ret + "'";
      if (errorMessage) *errorMessage = impl->lastError.c_str();
      return false;
   }

   impl->foreign[name] = decl;
   return true;
}
...
namespace
{
   ...
   struct Translator
   {
      ...
      // --- foreign calls ---
      //
      // The marshalling wrapper for a declared foreign function, emitted once
      // under the native's own selene.native symbol: unbox each argument per
      // the declared type, call the C symbol under the target's C ABI, and
      // return success. Phase 1 discards the native return value; it starts
      // travelling once fail: carries an argument (plan 18 section 8).
      llvm::Type* ffiNativeType(Impl::FFIType kind)
      {
         switch (kind) {
            case Impl::ffiVoid:  return llvm::Type::getVoidTy(impl.context);
            case Impl::ffiI8:
            case Impl::ffiU8:    return llvm::Type::getInt8Ty(impl.context);
            case Impl::ffiI16:
            case Impl::ffiU16:   return llvm::Type::getInt16Ty(impl.context);
            case Impl::ffiI32:
            case Impl::ffiU32:   return llvm::Type::getInt32Ty(impl.context);
            case Impl::ffiI64:
            case Impl::ffiU64:   return llvm::Type::getInt64Ty(impl.context);
            case Impl::ffiISize:
            case Impl::ffiUSize:
               return llvm::Type::getIntNTy(impl.context,
                                            impl.target->slotBytes() * 8);
            case Impl::ffiF64:   return llvm::Type::getDoubleTy(impl.context);
            default:             return ptrTy();          // ptr, str
         }
      }

      llvm::Function* foreignWrapper(const std::string& nativeName,
                                     const Impl::FFIDecl& decl)
      {
         std::string wrapperName =
            sanitizeSymbol("selene.native:" + nativeName);

         std::vector<llvm::Type*> params(decl.args.size(), ptrTy());
         llvm::FunctionType* wrapperType =
            llvm::FunctionType::get(impl.resultType, params, false);

         llvm::Function* fn = llvm::dyn_cast<llvm::Function>(
            impl.module->getOrInsertFunction(wrapperName, wrapperType)
               .getCallee());
         if (!fn) {
            fail("'%s' is called both typed and untyped", nativeName.c_str());
            return nullptr;
         }
         if (!fn->isDeclaration())
            return fn;                            // emitted by an earlier call

         llvm::IRBuilder<> b(
            llvm::BasicBlock::Create(impl.context, "entry", fn));

         std::vector<llvm::Type*>  nativeTypes;
         std::vector<llvm::Value*> nativeArgs;

         for (size_t i = 0 ; i < decl.args.size() ; i++) {
            llvm::Value* object = fn->getArg((unsigned int)i);
            Impl::FFIType kind  = decl.args[i];
            llvm::Type* natural = ffiNativeType(kind);

            llvm::Value* value;
            switch (kind) {
               case Impl::ffiPtr:
                  // An object pointer IS its payload address.
                  value = object;
                  break;

               case Impl::ffiStr:
                  // The literal payload is [u32 length][UTF-8 bytes][NUL];
                  // C gets the bytes.
                  value = b.CreateGEP(llvm::Type::getInt8Ty(impl.context),
                                      object,
                                      llvm::ConstantInt::get(i64Ty(), 4));
                  break;

               case Impl::ffiISize:
                  // Integer payloads are 32-bit; widen to the target word.
                  value = b.CreateSExt(
                     b.CreateLoad(llvm::Type::getInt32Ty(impl.context), object),
                     natural);
                  break;
               ...
               default:
                  // Sized integers and f64: the payload at its own width.
                  value = b.CreateLoad(natural, object);
            }

            nativeTypes.push_back(natural);
            nativeArgs.push_back(value);
         }

         llvm::FunctionCallee foreignFn = impl.module->getOrInsertFunction(
            decl.symbol,
            llvm::FunctionType::get(ffiNativeType(decl.ret), nativeTypes,
                                    false));

         b.CreateCall(foreignFn, nativeArgs);
         b.CreateRet(llvm::ConstantInt::get(impl.resultType, 1));
   ...
   bool Translator::translate(const char* fname, const unsigned char* code, size_t length)
   {
      ...
      for (size_t at = 0 ; at < instructions.size() ; at++) {
         ...
         switch (op) {
            ...
            case bcRCallExt:
            {
               // #external f(a, b, ...): the arguments were PUSHED left to
               // right, the call leaves the stack alone, and the byte code
               // pops them afterwards.
               //
               // With an [ffi] declaration the call is TYPED: the top K
               // slots marshal through the declared signature into a direct
               // call to the C symbol -- the syscall layer as metadata, not
               // code (plan 18). Without one, the legacy native convention
               // applies: the top two slots as (first-pushed, last-pushed).
               unsigned int naked = a1 & 0x00FFFFFFu;
               const char* resolved = impl.resolveName(naked);

               std::map<std::string, Impl::FFIDecl>::iterator ffi =
                  resolved ? impl.foreign.find(resolved)
                           : impl.foreign.end();

               if (ffi != impl.foreign.end()) {
                  int arity = (int)ffi->second.args.size();

                  llvm::Function* wrapper =
                     foreignWrapper(resolved, ffi->second);
                  if (!wrapper)
                     break;                       // reported by the helper

                  std::vector<llvm::Value*> args;
                  for (int i = 0 ; i < arity ; i++)
                     args.push_back(peek(arity - i));

                  llvm::Value* r = builder.CreateCall(wrapper, args);
